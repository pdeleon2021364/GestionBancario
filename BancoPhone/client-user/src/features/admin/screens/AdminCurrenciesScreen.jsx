...
const AdminCurrenciesScreen = () => {
    const { currencies, currenciesLoading, fetchCurrencies, createCurrency, updateCurrency, deleteCurrency } = useAdminStore();
    const [refreshing, setRefreshing] = useState(false);
    const [modalVisible, setModalVisible] = useState(false);
    const [editing, setEditing] = useState(null);
    const [form, setForm] = useState({ nombre: "", codigo: "", simbolo: "" });

    useFocusEffect(useCallback(() => { fetchCurrencies(); }, []));
    ...
    const openCreate = () => { setEditing(null); setForm({ nombre: "", codigo: "", simbolo: "" }); setModalVisible(true); };
    const openEdit = (item) => { setEditing(item); setForm({ nombre: item.nombre || "", codigo: item.codigo || "", simbolo: item.simbolo || "" }); setModalVisible(true); };

    const handleSave = async () => {
        if (!form.nombre.trim() || !form.codigo.trim()) { Alert.alert("Error", "Nombre y código son requeridos"); return; }
        try {
            if (editing) await updateCurrency(editing._id || editing.id, form);
            else await createCurrency(form);
            setModalVisible(false);
            Alert.alert("Éxito", editing ? "Divisa actualizada" : "Divisa creada");
    ...
    const handleDelete = (item) => {
        Alert.alert("Eliminar", `¿Eliminar ${item.nombre}?`, [
            ...
            { text: "Eliminar", style: "destructive", onPress: async () => { try { await deleteCurrency(item._id || item.id); Alert.alert("Eliminada"); } catch { Alert.alert("Error"); } } },
        ]);
    };
        ...
        <View style={styles.card}>
            <View style={styles.cardRow}>
                <View style={styles.iconCircle}>
                    <Text style={styles.iconText}>{item.simbolo || item.codigo?.charAt(0)}</Text>
                </View>
                <View style={styles.cardBody}>
                    <Text style={styles.cardTitle}>{item.nombre}</Text>
                    <Text style={styles.cardSub}>{item.codigo} {item.simbolo ? `· ${item.simbolo}` : ""}</Text>
    ...
    if (currenciesLoading && !refreshing) return <LoadingSpinner />;

    return (
        <View style={styles.container}>
            <FlatList
                data={Array.isArray(currencies) ? currencies : []}
                ...
                ListEmptyComponent={!currenciesLoading ? <EmptyState message="No hay divisas" /> : null}
            />
            <Modal visible={modalVisible} transparent animationType="slide" onRequestClose={() => setModalVisible(false)}>
                <View style={styles.modalOverlay}>
                    <View style={styles.modalContent}>
                        <View style={styles.modalHeader}>
                            <Text style={styles.modalTitle}>{editing ? "Editar Divisa" : "Nueva Divisa"}</Text>
                            <TouchableOpacity onPress={() => setModalVisible(false)}><MaterialIcons name="close" size={24} color={COLORS.text} /></TouchableOpacity>
                        </View>
                        <TextInput style={styles.input} placeholder="Nombre (ej. Quetzal)" placeholderTextColor={COLORS.textMuted} value={form.nombre} onChangeText={(t) => setForm({ ...form, nombre: t })} />
                        <TextInput style={styles.input} placeholder="Código (ej. GTQ)" placeholderTextColor={COLORS.textMuted} value={form.codigo} onChangeText={(t) => setForm({ ...form, codigo: t.toUpperCase() })} autoCapitalize="characters" maxLength={5} />
                        <TextInput style={styles.input} placeholder="Símbolo (ej. Q)" placeholderTextColor={COLORS.textMuted} value={form.simbolo} onChangeText={(t) => setForm({ ...form, simbolo: t })} maxLength={5} />
                        <Button title={editing ? "Actualizar" : "Crear Divisa"} onPress={handleSave} />
                    </View>
                </View>
            </Modal>
        </View>
...
const styles = StyleSheet.create({
    ...
    iconCircle: { width: 48, height: 48, borderRadius: 24, backgroundColor: COLORS.primary + "20", alignItems: "center", justifyContent: "center" },
    iconText: { fontSize: FONT_SIZE.lg, fontWeight: "700", color: COLORS.primary },
    cardBody: { flex: 1 },
    cardTitle: { fontSize: FONT_SIZE.md, fontWeight: "700", color: COLORS.text },
    cardSub: { fontSize: FONT_SIZE.sm, color: COLORS.textLight, marginTop: 2 },
...
export default AdminCurrenciesScreen;
