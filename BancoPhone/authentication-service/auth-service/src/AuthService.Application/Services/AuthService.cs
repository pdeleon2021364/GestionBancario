    ...
    ILogger<AuthService> logger) : IAuthService
{
    ...
    public async Task<UserResponseDto> UpdateProfileAsync(string userId, UpdateProfileDto dto)
    {
        ...
        if (dto.ProfilePicture != null && dto.ProfilePicture.Size > 0)
        {
            ...
            // Delete old picture from Cloudinary if it's not the default avatar
            var currentPicture = user.UserProfile.ProfilePicture;
            if (!string.IsNullOrEmpty(currentPicture) && currentPicture.StartsWith("v"))
            {
                try
                {
                    await _cloudinaryService.DeleteImageAsync(currentPicture);
                }
                catch (Exception ex)
                {
        ...
        await userRepository.UpdateAsync(user);
        logger.LogInformation("Profile updated for user {Username}", user.Username);
